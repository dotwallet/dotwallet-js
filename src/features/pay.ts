...
export function pay(elementID: string, options: PayOptions) {
	...
	const pay = (options: PayOptions) => {
		...
		if (options.log) console.log('sending order', { request: requestOptions });
		fetch(options.createOrderEndpoint, requestOptions)
	...
	button.addEventListener('click', () => {
		try {
			if (options.log) console.log('pay called with', { options });
			pay(options);
		} catch (error) {
			console.error({ error });
		}
